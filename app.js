...
function gestion_denvoie(){
    ...
    if(reponse!=="" && etape < question.length-1){
        ...
        setTimeout(()=>{
            ...
            add_message(question[etape],'recevoir');
            if(etape === 51){
            let pourcentage = (result / 50) * 100;
                let taux = Math.round(pourcentage);
                setTimeout(() => {
                    alert("Vous avez réussi " + result + " sur 50 questions. Votre taux de connaissance du Cameroun est de " + taux + "%");
                }, 500);
            }
        },700)
    }
